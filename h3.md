...
Aloitin luomalla uuden name-based virtual hostin Apache2:lle, joka ohjaa pyynnöt hattu.example.com-verkkotunnukseen. Tämän tekemiseksi loin asetustiedoston /etc/apache2/sites-available/hattu.example.com.conf, joka määrittelee, miten Apache käsittelee pyynnöt kyseiselle verkkotunnukselle. Tämän jälkeen otin virtual hostin käyttöön komennolla sudo a2ensite hattu.example.com.conf ja latasin Apache-palvelimen uudelleen komennolla sudo systemctl reload apache2.

Sitten määrittelin seuraavat asetukset: 

<img src="virtualhost.png" width="600" />

ServerName: Tässä määrittelen verkkotunnuksen, joka ohjataan tähän virtual hostiin. 
ServerAlias: Määritän mahdollisen vaihtoehtoisen verkkotunnuksen www.hattu.example.com, jotta molemmat versiot verkkotunnuksesta ohjautuvat oikeaan paikkaan.
DocumentRoot: Tämä on se hakemisto, jossa verkkosivun tiedostot sijaitsevat. Määritän sen /home/nita/publicsites/hattu.example.com -hakemistoksi, ja nita-muuttuja viittaa nykyisen käyttäjän kotihakemistoon.
Directory: Tällä osalla määritellään, että kaikille käyttäjille annetaan oikeus lukea ja käsitellä tiedostoja tässä hakemistossa. Tämä mahdollistaa sen, että pystyn itse muokkaamaan sivua ilman sudo-oikeuksia.

Kun olen luonut asetustiedoston, otan sen käyttöön Apache2:ssa komennolla sudo a2ensite hattu.example.com.conf

Seuraavaksi halusin poistaa Apache2:n oletusvirtuaali-isännän käytöstä, jotta etusivuksi ei tulisi vanha oletussivu. Käytin tähän komentoa sudo a2dissite 000-default.conf, mutta terminaali ei antanut jatkaa ennen kuin olin ensin aktivoinut uuden konfiguraation, eli käyttänyt komentoa sudo a2ensite hattu.example.com.conf.

<img src="sudo.png" width="600" />

En osannut mennä eteenpäin.
...
Kun aktivoin uuden konfiguraation, niin minun täytyi ladata Apache2:n asetukset uudelleen, jotta muutokset astuvat voimaan komennolla systemctl reload apache2. Tämän jälkeen koitin taas poistaa Apache2:n oletusvirtuaali-isännän käytöstä. 

Seuraavaksi koitin testata curl -H 'Host: hattu.example.com' localhost komennolla, mutta minä en saanut toimimaan. Tietääkö kukaan missä on mennyt mönkään?

<img src="youdont.png" width="600" />

---

Lähteet:

Veljeni auttoi ohjeisti minua tehtävässä
...
Curl -I lähettää HTTP HEAD -pyynnön ja palauttaa vain vastauksen otsaketiedot ilman varsinaista sisältöä. Esimerkki:
